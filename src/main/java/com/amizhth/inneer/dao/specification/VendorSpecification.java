...
public class VendorSpecification extends CommonSpecification implements Specification<VendorModel> {

	private List<FilterCriteria> criteriaList;

	private List<FilterCriteria> searchList;

	public VendorSpecification(List<FilterCriteria> filterCriterias, List<FilterCriteria> searchCriterias) {
		this.criteriaList = filterCriterias;
		this.searchList = searchCriterias;
	}

	@Override
	public Predicate toPredicate(Root<VendorModel> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
